...
(approximated by a discrete field) is known on a \textbf{donor} mesh and must
be transferred to a \textbf{target} mesh. In many applications, the field
must be conserved for physical reasons, e.g. mass or energy cannot leave or
enter the system, hence the focus on \textbf{conservative} solution transfer.
...
\begin{figure}
  \includegraphics{../images/solution-transfer/polygon_vs_curved_intersection.pdf}
  \centering
  \captionsetup{width=.75\linewidth}
...
the solution transfer on the cubic mesh should be \textbf{exact}, so there
